...
- [x] T003 Confirm semantic-release config `.releaserc.json` matches dry-run-only expectation (root/.releaserc.json)
...
**Goal**: CI computes next version via semantic-release dry-run in bump_version stage; existing commit stage pushes versioned files.

**Independent Test**: Run bump_version stage; semantic-release computes version, package.json updated; commit stage can push.

### Implementation for User Story 1

- [x] T008 [US1] Ensure `cicd/Dockerfile.production` bump_version uses `pnpm release --ci --dry-run --no-verify` and writes computed version to package.json (cicd/Dockerfile.production)
- [x] T009 [US1] Validate git fetch tags and error handling for missing computed version in bump_version stage (cicd/Dockerfile.production)
- [x] T010 [US1] Confirm commit_new_version stage consumes package.json from bump_version and remains unchanged elsewhere (cicd/Dockerfile.production)
...
**Checkpoint**: bump_version computes version; commit_new_version stage can push.
...
- [ ] T014 [US2] Add/update contributor docs snippet for acceptable commit types and breaking-change syntax (README.md or docs/release.md)
...
- [x] T015 [US3] Document release flow (dry-run in bump_version + existing commit stage) and required env vars (README.md or docs/release.md)
