...
- CI executes semantic-release inside the `bump_version` target in `cicd/Dockerfile.production`; semantic-release performs the release (computes version, updates configured assets, creates the release commit/tag) and pushes to GitHub.
- The semantic-release git commit message MUST include `[skip ci]` so the follow-up Jenkins run (triggered by the push) skips CI stages.
...
- **FR-002**: Project MUST configure semantic-release with conventional-commits preset and plugins for changelog and git tagging/commits; when invoked in CI `bump_version` stage it MUST run in CI mode and perform the release commit/tag push according to `.releaserc.json`.
