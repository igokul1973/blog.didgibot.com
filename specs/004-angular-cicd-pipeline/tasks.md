...
- [x] T001 Review existing CI/CD assets to understand current behaviour in `cicd/Jenkinsfile`, `cicd/Dockerfile`, `cicd/Dockerfile.node`, `cicd/Dockerfile.production`, and `cicd/node-git-chromium-pod.yaml`.
...
**Purpose**: Establish the Node-based Jenkins agent image and Kubernetes agent pod before changing the pipeline.
...
- [ ] T006 [P] On an AMD64 Linux host, build the Jenkins agent image from `cicd/Dockerfile.production` and push it to the registry as `igk19/node-22:1.0.0` using the documented command in `specs/004-angular-cicd-pipeline/quickstart.md`.
- [x] T007 [P] Update the Kubernetes agent pod manifest in `cicd/node-git-chromium-pod.yaml` so the `node-git-chromium` container uses `igk19/node-22:1.0.0` while preserving the existing `kaniko` container configuration.

**Checkpoint**: Jenkins agent pod can run Node 22 + git + Chromium + `pnpm` using the new prebuilt agent image.
...
- [ ] T008 [US1] Create a new pipeline definition `cicd/Jenkinsfile_test` that uses the Kubernetes agent pod from `cicd/node-git-chromium-pod.yaml` (agent + `kaniko`) and defines a clear stage structure (e.g., Prepare, Lint, Test, Build, Image Build/Push).
- [ ] T009 [P] [US1] Wire all build, lint, and test steps in `cicd/Jenkinsfile_test` to use `pnpm` and scripts from `package.json` (no ad-hoc shell commands beyond invoking these scripts).
- [ ] T010 [P] [US1] Add a dedicated `testing` stage in `cicd/Jenkinsfile_test` that runs `pnpm test:coverage` for the Angular frontend and ensures coverage output is written to a known directory (e.g., `coverage/`).
- [ ] T011 [US1] Configure `cicd/Jenkinsfile_test` to publish the coverage report as a visible Jenkins artifact (e.g., via HTML or coverage publisher plugin) so it can be opened from the Jenkins UI.
- [ ] T012 [US1] Implement coverage enforcement logic in `cicd/Jenkinsfile_test` (or a called script) so that the pipeline fails if any non-excluded file is below 90% test coverage, respecting `vite.config.ts` exclusions.
- [ ] T013 [P] [US1] Add a build stage in `cicd/Jenkinsfile_test` that runs the Angular build via the appropriate `pnpm` script from `package.json` and produces the final `dist/` output required by `cicd/Dockerfile`.
- [ ] T014 [US1] Implement an image build/push stage in `cicd/Jenkinsfile_test` that uses the `kaniko` container from `cicd/node-git-chromium-pod.yaml` to build and push the Angular application Docker image, mirroring the current behaviour in `cicd/Jenkinsfile`.
- [ ] T015 [US1] Add clear stage-level error handling and logging in `cicd/Jenkinsfile_test` so that failures in linting, testing/coverage, build, or image push are visible and traceable from Jenkins.
...
- [ ] T016 [US2] Update or create a Jenkins job configuration (via Jenkins UI) to use `cicd/Jenkinsfile_test` as its pipeline definition and to run the job inside the Kubernetes agent pod defined in `cicd/node-git-chromium-pod.yaml`.
- [ ] T017 [US2] Configure that Jenkins job so that, for the testing phase, it is triggered on pushes/merges to both the `main` branch and the `004-angular-cicd-pipeline` branch (Jenkins job configuration, not Jenkinsfile logic).
- [ ] T018 [P] [US2] Push a safe change to the `004-angular-cicd-pipeline` branch and verify in Jenkins that all stages in `cicd/Jenkinsfile_test` run successfully, coverage artifacts are published, and the Angular application image is built and pushed.
- [ ] T019 [P] [US2] Perform negative coverage validations on the `004-angular-cicd-pipeline` branch:
...
- [ ] T020 [US3] Rename the existing primary Jenkinsfile from `cicd/Jenkinsfile` to `cicd/Jenkinsfile_deprecated` and add a header comment explaining that it is deprecated and retained only for historical reference and rollback.
- [ ] T021 [US3] Rename `cicd/Jenkinsfile_test` to `cicd/Jenkinsfile` and adjust any internal comments or references within that file to reflect its new canonical role.
- [ ] T022 [US3] Update the existing Jenkins job configuration (via Jenkins UI) to point to the new `cicd/Jenkinsfile` as the pipeline definition and remove any special triggers for the `004-angular-cicd-pipeline` branch so that only the desired long-term branches (e.g., `main`) trigger the job.
- [ ] T023 [US3] Update `specs/004-angular-cicd-pipeline/quickstart.md` and any relevant CI/CD documentation in `README.md` or `development_guidelines.md` to describe the new canonical Jenkinsfile and deprecation status of the old one.
...
- [ ] T024 [P] Verify that all CI/CD-related files in `cicd/` (Jenkinsfiles, Dockerfiles, pod manifests) follow repository linting/formatting conventions and do not introduce new lint errors.
- [ ] T025 [P] Capture a "first successful run" record (screenshots or notes) for the new pipeline and link it from `specs/004-angular-cicd-pipeline/quickstart.md` or `README.md` as a validation reference.
- [ ] T026 [P] Review `specs/004-angular-cicd-pipeline/spec.md`, `plan.md`, `research.md`, `data-model.md`, `contracts/README.md`, and `quickstart.md` for consistency with the final implementation and update any outdated details.
- [ ] T027 Run a final end-to-end test by triggering the Jenkins job on the long-term target branch (e.g., `main`) to confirm that the agent image, coverage enforcement, Angular build, and Docker image push all behave as expected.
...
- **User Story 2 (P2)**: Depends on User Story 1 (needs working Jenkinsfile_test and agent image setup).
