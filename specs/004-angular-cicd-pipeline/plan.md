...
Implement a Jenkins/Kubernetes-based CI/CD pipeline for the Angular frontend that runs on a Node-based Jenkins agent image, enforces 90% per-file test coverage for all non-excluded files, builds and pushes the Angular application Docker image via a containerized image builder (e.g., kaniko), and deprecates the old pipeline while keeping a clear migration and rollback path.
