...
- Jenkins jobs must be configured to:
    ...
    - Run using the Kubernetes agent pod defined in `cicd/node-git-chromium-pod.yaml`.
    - Configure which branches or refs trigger the pipeline (the Jenkinsfile remains branch-agnostic).

## Jenkins agent image contract

- The Kubernetes agent pod must reference a prebuilt image based on `cicd/Dockerfile.production`, for example:
    - `image: igk19/node-22:1.0.0`
- The agent image must provide:
    - Node.js 22 (aligned with the project).
    - `pnpm` installed globally.
    - `git` and `chromium` available for tests and tooling.
