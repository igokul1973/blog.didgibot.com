...
- Q: How should the CI Jenkins agent Docker image used in the Kubernetes pod be built and tagged? → A: Create a Node-based agent image `igk19/node-22:1.0.0` manually for running on a AMD64 Linux machine from `cicd/Dockerfile.production`, push it once to the Docker registry, configure the Jenkins Kubernetes agent pod to use that tag, and ensure the CI/CD pipeline does not build or publish this agent image.
...
- **FR-010**: A new `cicd/Dockerfile.production` MUST be created for the Jenkins Kubernetes agent image, using Node.js (re-using the Node version and core tooling assumptions from `cicd/Dockerfile.node`, such as git, Chromium, and `pnpm`) instead of the current Python-based example. This Dockerfile MUST be suitable for building the `igk19/node-22:1.0.0` Jenkins agent image manually (for AMD64 Linux host) outside any CI/CD pipeline, to be used by the Jenkins Kubernetes agent pod.
- **FR-011**: The current pod manifest `cicd/node-git-chromium-pod.yaml` MUST be updated to use the newly-created image from the properly modified `cicd/Dockerfile.production`.
...
- **CI Jenkins Agent Docker Image**: Prebuilt Docker image (for example, `igk19/node-22:1.0.0`) created manually from `cicd/Dockerfile.production` and used by the Jenkins Kubernetes agent pod to run the Angular CI/CD pipeline; this image is not built or published by the CI/CD pipeline itself.
